...
function AddtoCart(name, description, price) { //Adding to  cart
    let singleProduct = {};
    ...
    let cart_total_price = 0; //Displaying total in an alert
    for (let product in shoppingCart) {
        let row = orderedProductsTblBody.insertRow();
        let cellName = row.insertCell(0);
        let cellDescription = row.insertCell(1);
...
function confirmOrder() { //An alerts to let user know their order was successful and generates a reference number
